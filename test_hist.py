...
if __name__ == "__main__":
  ...
  trade_file = os.path.join(current_dir, "IC2311_trade.csv").replace("\\", "/")

  s.run(f"tt = loadText('{tick_file}')")
  s.run(f"td = loadText('{trade_file}')")
  ...
  s.run("""
    for (i in 0..50){
      if (tt.slice(i).tradetime in td.tradetime){
        ...
        trades.append!(temp_)
        print("Trade processed at index " + string(i))
        sleep(100)
      }
      insert into tickStream values(tt.slice(i).values())
      if (i % 10 == 0) print("Processed " + string(i) + " tick records")
      sleep(10)
    }
  """)
  ...
  print(f"Trades processed: {trades_count}")
