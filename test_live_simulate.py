...
class ctp_gateway(CtpGateway):
  ...
  def connect(self, setting):
    ...
    print(f"Initiating connection to TD server: {td_address}")
    self.td_api.connect(td_address, userid, password, brokerid, auth_code, appid)
    print(f"Initiating connection to MD server: {md_address}")
...
if __name__ == "__main__":
  ...
  print("Waiting for CTP connection and login...")
  connected = False
  for i in range(20):  # wait up to 20 seconds
      td_status = gateway.td_api.login_status
      md_status = gateway.md_api.login_status
      if td_status and md_status:
          print("CTP connection established and logged in.")
          connected = True
          break
      print(f"Waiting... TD login: {td_status}, MD login: {md_status}")
      time.sleep(1)

  if not connected:
      print("Could not establish CTP connection. Please check settings and network. Exiting.")
      stop_event.set()
      gateway.close()
      time.sleep(5)
      s.close()
  ...
  try:
    print("Script is running. It will stop automatically after 2 minutes.")
    time.sleep(120)
    print("Two minutes have passed. Shutting down...")
    ...
    s.close()
    print("DolphinDB session closed. Shutdown complete.")